  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {isVerifying ? (
          <div className="space-y-4 py-2">
            <div className="space-y-1.5 text-center mb-6">
              ...
              <div className="text-white font-black">{formData.email}</div>
            </div>
            
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest text-center block mb-2">Verification Code</label>
              <input
                type="text"
                placeholder="123456"
                value={otpCode}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    style={inputStyle}
                  />
                  <button type="button" onClick={() => setShowPwd(!showPwd)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-white transition-colors">
                    {showPwd ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Confirm</label>
                <input
                  type={showPwd ? 'text' : 'password'}
                  placeholder="••••••••"
        ...
        <motion.button
          type="submit"
          disabled={loading || (isVerifying && otpCode.length !== 6)}
        ...
        <>
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="bg-[#111] px-2 text-muted-foreground">or</span>
            </div>
          </div>
