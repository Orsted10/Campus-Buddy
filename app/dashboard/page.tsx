...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8 pb-safe md:pb-8 overflow-x-hidden pt-safe">
      {/* 1. HERO SECTION */}
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-4 md:space-y-2">
            ...
            <motion.h1 
              ...
              className="text-4xl md:text-6xl font-black text-foreground tracking-tighter leading-none mobile-comfy"
            >
              {greeting.text}, <span className="text-gradient underline decoration-primary/20">{user?.full_name?.split(' ')[0] || 'Buddy'}</span>
            </motion.h1>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-4 text-muted-foreground font-bold text-sm md:text-sm mobile-comfy">
                <span className="flex items-center gap-1.5"><CalendarIcon className="w-4 h-4" /> {currentTime.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', timeZone: 'UTC' })}</span>
